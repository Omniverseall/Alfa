...
export const adminService = {
  ...
  // News
  getNews: async (): Promise<NewsItem[]> => fetchWithCache('news', async () => {
  ...
  addNews: async (news: Omit<NewsItem, 'id'>): Promise<NewsItem | null> => {
    try {
      ...
      if (error) {
        ...
        if (error.code === '23502') {
             console.error('ОШИБКА: Нарушено ограничение NOT NULL. Проверь схему БД!');
        }
        throw error;
      }
      adminService.clearCache('news');
      adminService.getNews().catch(err => console.error("Failed refetch after adding news:", err));
      return data;
    } catch (error) {
      return null;
    }
  ...
  updateNews: async (id: string, news: Partial<Omit<NewsItem, 'id'>>): Promise<NewsItem | null> => {
    try {
       if (news.image !== undefined) news.image = news.image || null;
       const { data, error } = await supabase
        .from('news')
        .update(news)
        .eq('id', id)
      ...
      if (error) throw error;
      adminService.clearCache('news');
      adminService.getNews().catch(err => console.error("Failed refetch after updating news:", err));
      return data;
    } catch (error) {
  ...
  deleteNews: async (id: string): Promise<boolean> => {
     try {
      ...
      if (error) throw error;
      adminService.clearCache('news');
      adminService.getNews().catch(err => console.error("Failed refetch after deleting news:", err));
      return true;
    } catch (error) {
 ...
 subscribeNews: (callback: (news: NewsItem[]) => void): (() => void) => {
      ...
      if (memoryCache.news.length > 0 && isMemoryCacheValid('news')) {
          callback([...memoryCache.news]);
      } else if (memoryCache.news.length === 0 && isMemoryCacheValid('news')) {
           callback([]);
      } else {
          adminService.getNews().catch(err => console.error("Initial fetch failed for news subscription:", err));
      }
